function spinalCase(str) {
	...
	masterString += strArray[0].toLowerCase();
	for (let i = 1; i < strArray.length; i++) {
		if (strArray[i].search(/[A-Z]/) != -1) {
			masterString += "-";
			masterString += strArray[i].toLowerCase();
		} else if (
			strArray[i].search(/[a-z]/) != -1 &&
			strArray[i - 1].search(/[A-Za-z]/) == -1
		) {
			masterString += "-";
			masterString += strArray[i];
		} else if (strArray[i].search(/[a-z]/) != -1) {
			masterString += strArray[i];
		}
	}
